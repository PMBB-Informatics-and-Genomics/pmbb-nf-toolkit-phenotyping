...
Coverage:
  ...
  - detect_concept_from_values()
  - detect_concept_from_column_name()
"""
...
class TestNormalizeConcept:
    ...
    def test_unknown_concept_passthrough(self):
        assert normalize_concept('CUSTOM_CODE') == 'CUSTOM_CODE'

    def test_leading_trailing_whitespace_stripped(self):
        assert normalize_concept('  ICD09  ') == 'ICD9'

    def test_non_string_integer(self):
        # Non-strings should not raise; returned as string
        result = normalize_concept(42)
...
class TestDetectConceptFromValues:
    ...
    def test_empty_values_returns_unknown(self):
        assert detect_concept_from_values([]) == 'unknown'

    def test_all_nan_returns_unknown(self):
        import pandas as pd
        assert detect_concept_from_values([None, None, float('nan')]) == 'unknown'

    def test_categorical_values(self):
        values = ['male', 'female', 'unknown', 'male', 'female']
        result = detect_concept_from_values(values)
...
class TestDetectConceptFromColumnName:
    ...
    def test_known_column_names(self, col, expected):
        assert detect_concept_from_column_name(col) == expected

    def test_unknown_column_returns_none(self):
        assert detect_concept_from_column_name('SOME_RANDOM_COL') is None

    def test_empty_string_returns_none(self):
        assert detect_concept_from_column_name('') is None

    def test_case_insensitive_keyword_matching(self):
        assert detect_concept_from_column_name('bmi') == 'quantitative'
        assert detect_concept_from_column_name('sex') == 'demographic'
