...
def test_run_preprocess_quantitative(tmp_path, pmbb_vitals_bmi):
    ...
    assert set(df.columns) >= {'sample_ID', 'concept', 'phenotype', 'value', 'occurrence_date'}
    assert all(df['phenotype'] == 'BMI')
    assert all(df['concept'] == 'quantitative')
...
def test_run_preprocess_diagnostic_all(tmp_path, pmbb_icd):
    cfg = _make_config(tmp_path, {
...
def test_run_preprocess_diagnostic_specific_codes(tmp_path, pmbb_icd):
    cfg = _make_config(tmp_path, {
...
def test_run_preprocess_sample_list_filters_diagnostic(tmp_path, pmbb_test_dir):
    """sample_list is applied before diagnostic code explosion."""
