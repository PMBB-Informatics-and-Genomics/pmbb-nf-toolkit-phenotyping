...
from diagnostic_basic import run_diagnostic_basic


def _make_long_tsv(tmp_path, code, rows):
    """rows: list of (sample_ID, occurrence_date)"""
    data = [{'sample_ID': sid, 'concept': 'diagnostic', 'phenotype': code,
...
def test_present_sample_gets_1(tmp_path):
    ...
    run_diagnostic_basic(tsv, cfg, out, all_samples=['P001', 'P002', 'P003'])
...
def test_absent_sample_na_by_default(tmp_path):
    ...
    out = str(tmp_path / 'N80.0.diag.tsv')
    run_diagnostic_basic(tsv, cfg, out, all_samples=['P001', 'P002'])
    df = pd.read_csv(out, sep='\t')
...
def test_output_column_named_after_code(tmp_path):
    ...
    run_diagnostic_basic(tsv, cfg, out, all_samples=['P001'])
    df = pd.read_csv(out, sep='\t')
