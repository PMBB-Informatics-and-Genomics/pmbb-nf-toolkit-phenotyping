...
include { DIAGNOSTIC_BASIC }    from './modules/diagnostic_basic/main.nf'
include { DIAGNOSTIC_ADVANCED } from './modules/diagnostic_advanced/main.nf'
...
workflow {
    ...
    log.info([
        ...
        String.format("  %-25s : %s", "--diagnostic_chunk_size", params.diagnostic_chunk_size ?: 0),
    ...
    //   RESOLVE_CONFIG emits separate channels per concept type.
    ...
    diag_jsons  = RESOLVE_CONFIG.out.diag_configs.flatten()
    other_jsons = RESOLVE_CONFIG.out.quant_configs
        .mix(RESOLVE_CONFIG.out.cat_configs)
        .flatten()

    PREPROCESS_DIAG(diag_jsons, ch_sample_list)
    PREPROCESS_OTHER(other_jsons, ch_sample_list)

    // Expand multi-file diagnostic outputs into individual (json, tsv) pairs
    preprocessed_pairs = PREPROCESS_DIAG.out.with_config
        .mix(PREPROCESS_OTHER.out.with_config)
        .transpose()

    // Tag each basic diagnostic long.tsv with its phenotype_name
    ...
    if (params.event_mask_config) {
        ...
        preprocessed_pairs
            .map { cfg_json, tsv ->
                ...
                [needs_mask, cfg.concept, cfg_json, tsv]
            }
            .branch {
                masked:   it[0] == true
                unmasked: true
        ...
        APPLY_MASK(
            mask_routing.masked.map { nm, concept, json, tsv -> [json, tsv] },
            mask_intervals_ch
        )

        masked_concept_pairs = APPLY_MASK.out.with_config
            .map { cfg_json, tsv ->
                def concept = new groovy.json.JsonSlurper().parseText(cfg_json.text).concept
                [concept, cfg_json, tsv]
            }

        unmasked_concept_pairs = mask_routing.unmasked
            .map { nm, concept, json, tsv -> [concept, json, tsv] }

        all_concept_pairs = masked_concept_pairs.mix(unmasked_concept_pairs)

    } else {
        all_concept_pairs = preprocessed_pairs
            .map { cfg_json, tsv ->
                def concept = new groovy.json.JsonSlurper().parseText(cfg_json.text).concept
                [concept, cfg_json, tsv]
            }
    ...
    // -------------------------------------------------------------------------
    DIAGNOSTIC_BASIC(diag_pairs)

    // -------------------------------------------------------------------------
    // Step 3b: Advanced diagnostic phenotypes
    //   Each advanced phenotype references one or more basic diagnostic
    ...
    advanced_inputs = RESOLVE_CONFIG.out.advanced_diag_configs
        .flatten()
    ...
    DIAGNOSTIC_ADVANCED(advanced_inputs)

    diag_results = DIAGNOSTIC_BASIC.out.result
        .mix(DIAGNOSTIC_ADVANCED.out.result)
    ...
    //   Config is defined in pmbb_config.yaml under concepts.quantitative
