#!/usr/bin/env python3
"""
diagnostic_advanced.py — Named phenotype from multiple diagnostic long.tsvs.

Reads long.tsvs from one or more basic diagnostic phenotypes, pools all codes
...
def run_diagnostic_advanced(long_tsvs, config_path, output_path, all_samples=None):
    """
    Compute a single named phenotype column from multiple diagnostic long.tsvs.
    ...
    case_codes      = normalize_patterns([c for s in sources.values() for c in (s.get('case_codes') or [])])
    case_exclude    = normalize_patterns([c for s in sources.values() for c in (s.get('case_exclude') or [])])
    control_exclude = normalize_patterns([c for s in sources.values() for c in (s.get('control_exclude') or [])])
...
def main():
    ...
    run_diagnostic_advanced(args.input, args.config, args.output, all_samples)
