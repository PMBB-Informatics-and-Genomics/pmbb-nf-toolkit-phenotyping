...
CONCEPT_TYPES = {'quantitative', 'categorical', 'diagnostic', 'advanced_diagnostic'}
...
def merge_params(global_params, concept_params, pheno_params, concept):
    """Merge: built-in concept defaults → global → concept → phenotype."""
    if concept == 'quantitative':
        base = deepcopy(QUANT_DEFAULTS)
    elif concept == 'categorical':
        base = deepcopy(CAT_DEFAULTS)
    elif concept == 'advanced_diagnostic':
        base = deepcopy(ADVANCED_DIAG_DEFAULTS)
    else:
        base = deepcopy(DIAG_DEFAULTS)

    base.update(deepcopy(GLOBAL_DEFAULTS))
    base.update(deepcopy(global_params))
    base.update(deepcopy(concept_params))
...
def validate(name, params):
    ...
    if concept not in CONCEPT_TYPES:
        errors.append(f"'concept' must be one of {sorted(CONCEPT_TYPES)}, got '{concept}'")
        return errors

    if concept == 'advanced_diagnostic':
        sources = params.get('sources') or {}
        if not sources:
            errors.append("'sources' is required for advanced_diagnostic phenotypes")
        ...
        if '.' in name:
            print(
                f"WARNING: advanced_diagnostic phenotype '{name}' contains a dot. "
...
def _get_diagnostic_codes(resolved):
    """Read the diagnostic table and return unique codes after applying filter and subsample.
...
def resolve(config_path, output_dir, diagnostic_chunk_size=0):
    ...
    concepts_block = config.get('concepts', {}) or {}
    ...
    for name, pheno_params in phenotypes.items():
        ...
        concept = pheno_params.get('concept') or global_params.get('concept')
        if not concept:
            print(f"ERROR: phenotype '{name}' has no concept", file=sys.stderr)
            sys.exit(1)

        concept_params = concepts_block.get(concept, {}) or {}
        resolved = merge_params(global_params, concept_params, pheno_params, concept)
        ...
        out_subdir = os.path.join(output_dir, concept)
        os.makedirs(out_subdir, exist_ok=True)

        # advanced_diagnostic: no table paths to resolve, no chunking — write JSON as-is
        if concept == 'advanced_diagnostic':
        ...
        if concept == 'diagnostic' and diagnostic_chunk_size > 0 and not resolved.get('preprocessed_path'):
            codes = _get_diagnostic_codes(resolved)
            chunks = [codes[i:i + diagnostic_chunk_size]
                      for i in range(0, len(codes), diagnostic_chunk_size)]
...
def main():
    ...
    p.add_argument('--diagnostic_chunk_size', type=int, default=0,
                   help='Split diagnostic phenotypes into chunks of this size (0 = disabled)')
    args = p.parse_args()
    resolve(args.config, args.output_dir, diagnostic_chunk_size=args.diagnostic_chunk_size)
