#!/usr/bin/env python3
"""
Shared utilities for ICD code matching, concept normalization, and auto-detection.
...
def normalize_concept(concept: str) -> str:
    """
    Normalize concept labels to canonical form.
    ...
    Unknown concepts are returned as-is (uppercased).
    """
...
def normalize_patterns(patterns: list | None) -> list:
    """
    Normalize ICD pattern lists for prefix matching.

    Bare codes (no dot, no wildcard) get '.*' appended so they prefix-match
    all subcodes: 'E11' → 'E11.*' matches 'E11.9', 'E11.65', etc.
    ...
    for p in patterns:
        p = str(p).strip()
        if p and '*' not in p and not p.endswith('.') and '.' not in p:
            p = p + '.*'
        if p:
            normalized.append(p)
    return normalized
...
def detect_concept_from_values(values, sample_size: int = 50) -> str:
    """
    Heuristically detect concept type from a sample of values.
...
def detect_concept_from_column_name(col_name: str) -> str | None:
    """
    Heuristically detect concept type from a column name.
