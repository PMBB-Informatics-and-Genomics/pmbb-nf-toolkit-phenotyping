...
def build_long(df, cfg, phenotype_name_override=None):
    ...
    Columns: sample_ID, concept, phenotype, value, occurrence_date
    """
...
def apply_outlier_filter(df: pd.DataFrame, method: str, iqr_mult: float,
    ...
    """
    report = {}
    quant_mask = df['concept'] == 'quantitative'
...
def _preprocess_diagnostic(cfg, table_path, output_dir, allowed_samples):
    """Stream diagnostic table in chunks, writing per-code TSVs without loading full table."""
    value_col = cfg['value_col']
    sample_id_col = cfg['sample_id_col']
...
def run_preprocess(config_path, output_dir, sample_list_path=None):
    ...
    if concept == 'diagnostic':
        _preprocess_diagnostic(cfg, table_path, output_dir, allowed_samples)
        return
    ...
    if concept == 'quantitative' and cfg.get('outlier_method', 'none') != 'none':
    ...
    if concept == 'quantitative' and cfg.get('scale'):
