...
For each quantitative phenotype:
  1. Subset rows where data_type == 'quantitative' (or 'measurements') and
     phenotype == <target>
...
QUANTITATIVE_DATA_TYPES = {'quantitative', 'measurements', 'vitals'}
...
def _legacy_main():
    ...
    if 'data_type' not in df.columns:
        print("WARNING: No 'data_type' column found. Processing all rows as quantitative.",
              file=sys.stderr)
        quant_df = df.copy()
    else:
        quant_mask = df['data_type'].str.lower().isin(QUANTITATIVE_DATA_TYPES)
